...
export function CanvasPreviewSurface(props: CanvasPreviewSurfaceProps) {
  ...
  const containerClass = `p-xl flex-1 overflow-auto bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:20px_20px]${containerCursorClass ? ` ${containerCursorClass}` : ""}`;
    ...
    <div ref={scrollContainerRef} className={containerClass}>
      <div data-canvas-wrapper className="relative mx-auto" style={{ width: `${viewportWidth * scale}px` }}>
        <CanvasPreviewToolbar
          ...
          onPickerAction={(action) => sendPickerAction(iframeRef.current, selectedMpId, action)}
        />
