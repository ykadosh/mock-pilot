...
export function CaptureBrowser() {
  ...
  const handleExtendPreview = useCallback(async (targetHeight: number) => {
    ...
    state.setPreviewBusy({ active: true, message: "Extending preview…" });
    try {
      ...
      state.setPreviewBusy({ active: false, message: "" });
    }
  }, [state]);
