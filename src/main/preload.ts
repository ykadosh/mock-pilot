...
contextBridge.exposeInMainWorld("api", {
  ...
  captureWebviewFullPage: (args: { webContentsId: number; width: number; height: number; deviceScaleFactor?: number }) =>
    ipcRenderer.invoke("capture-webview-full-page", args) as Promise<{ success: boolean; dataUrl?: string; error?: string }>,
  ...
  // AI Agent
  aiAgentModify: (data: { prompt: string; fullHTML: string; projectId?: string; sessionId?: string; attachedElements?: { mpId: string; selector: string; outerHTML: string }[]; images?: { name: string; dataUrl: string }[]; attachedAssets?: unknown; projectAssets?: object; previousAgentMessages?: unknown[]; continueFromMaxIterations?: boolean }) =>
